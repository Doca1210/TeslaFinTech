...
def build_html_report(report_dict: dict) -> str:
    ...
    for rank, (score, v) in enumerate(ranked, start=1):
        ...
        <div class="{best_class}">
          ...
          <div class="bar-label">Composite score</div>
          {bar(score)}
          <table class="mini">
            <tr><td>Detection Rate</td><td><strong>{pct(fm['recall'])}</strong></td>
                <td>Miss Rate</td><td class="{'red-txt' if fm['false_negative_rate'] > 0.05 else ''}">{pct(fm['false_negative_rate'])}</td></tr>
            <tr><td>False Alarm Rate</td><td>{pct(fm['false_positive_rate'])}</td>
                <td>Alert Precision</td><td>{pct(fm['precision'])}</td></tr>
            <tr><td>F2 Score</td><td>{fm['f2_score']:.3f}</td>
                <td>MCC</td><td>{fm['mcc']:.3f}</td></tr>
            <tr><td>Alert Rate</td><td>{pct(fm['alert_rate'])}</td>
                <td>Speed</td><td>{v['avg_latency_ms']:.1f} ms</td></tr>
          </table>
    ...
    def metric_row(label: str, vals: list[str]) -> str:
        cells = ""
        for val in vals:
            cells += f"<td>{val}</td>"
    ...
    <table class="comparison">
      ...
      <tbody>
        {metric_row("Detection Rate (Recall)", [pct(v["flag_metrics"]["recall"]) for _, v in ranked])}
        {metric_row("Miss Rate ← compliance risk", [pct(v["flag_metrics"]["false_negative_rate"]) for _, v in ranked])}
        {metric_row("False Alarm Rate", [pct(v["flag_metrics"]["false_positive_rate"]) for _, v in ranked])}
        {metric_row("Alert Precision", [pct(v["flag_metrics"]["precision"]) for _, v in ranked])}
        {metric_row("F2 Score", [f"{v['flag_metrics']['f2_score']:.3f}" for _, v in ranked])}
        {metric_row("MCC", [f"{v['flag_metrics']['mcc']:.3f}" for _, v in ranked])}
        {metric_row("F1 Score", [f"{v['flag_metrics']['f1_score']:.3f}" for _, v in ranked])}
        {metric_row("Accuracy", [pct(v["flag_metrics"]["accuracy"]) for _, v in ranked])}
        {metric_row("Alert Rate", [pct(v["flag_metrics"]["alert_rate"]) for _, v in ranked])}
        {metric_row("Auto-Block Rate", [pct(v["block_metrics"]["alert_rate"]) for _, v in ranked])}
        {metric_row("Entity Hit Rate", [pct(v["entity_hit_rate"]) if v.get("entity_hit_rate") is not None else "n/a" for _, v in ranked])}
        {metric_row("Speed (ms / txn)", [f"{v['avg_latency_ms']:.1f} ms" for _, v in ranked])}
      </tbody>
    </table>"""
    ...
    for rank, (score, v) in enumerate(ranked, start=1):
        ...
        <div class="cm-block">
          ...
          <table class="cm">
            <tr>
              <td class="tp">✔ Correct Alerts<br><strong>{fm['true_positives']}</strong><br><small>(TP)</small></td>
              <td class="fn">✘ Missed Hits<br><strong>{fm['false_negatives']}</strong><br><small>(FN) ← risk</small></td>
            </tr>
            <tr>
              <td class="fp">⚠ False Alarms<br><strong>{fm['false_positives']}</strong><br><small>(FP)</small></td>
              <td class="tn">✔ Clean Passes<br><strong>{fm['true_negatives']}</strong><br><small>(TN)</small></td>
            </tr>
          </table>
        </div>"""
    ...
    <div class="winner">
      ...
      <div class="config-grid">
        {''.join(f"<div><span>{k}</span><strong>{v}</strong></div>" for k, v in best.get('config', {}).items())}
      </div>
    </div>"""
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    h2 { font-size: 1.1rem; font-weight: 600; color: #334155; margin: 40px 0 16px; border-bottom: 2px solid #e2e8f0; padding-bottom: 8px; }
    ...
    .bar { height: 10px; border-radius: 6px; transition: width 0.3s; }
    ...
    .badge { display: inline-block; font-size: 0.7rem; font-weight: 700; padding: 2px 8px; border-radius: 99px; margin-top: 10px; }
    ...
    .cm { border-collapse: collapse; }
    .cm td { width: 140px; height: 80px; text-align: center; border-radius: 8px; font-size: 0.8rem; padding: 8px; }
    ...
    .config-grid { display: flex; gap: 24px; flex-wrap: wrap; margin-top: 20px; border-top: 1px solid #334155; padding-top: 16px; }
...
<body>
  ...
  <h2>Ranking</h2>
  <p style="font-size:0.8rem;color:#64748b;margin-bottom:8px">Composite score = 60% F2 (recall-weighted) + 40% MCC (robust for imbalanced data)</p>
